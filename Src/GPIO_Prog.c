...
ES_t GPIO_enuSetPinValue(GPIO_PinConfig_t*Copy_PsPinConfig,Pin_Value_t Copy_enuValue)
{
	ES_t LoclaErrorState = ES_OK;
	if(Copy_PsPinConfig!=NULLPTR)
	{
		if(Copy_PsPinConfig->PortId>=PORTA && Copy_PsPinConfig->PortId<=PORTC)
		{
			switch(Copy_enuValue)
			{
			  case LOW:
			  	CLEAR_BIT(GPIO_APS[Copy_PsPinConfig->PortId]->GPIO_BRR,Copy_PsPinConfig->PinId);
			  	break;
			  case HIGH:
			  	SET_BIT(GPIO_APS[Copy_PsPinConfig->PortId]->GPIO_BSRR,Copy_PsPinConfig->PinId);
			  	break;
			  default:
			  	LoclaErrorState = ES_NOK;
